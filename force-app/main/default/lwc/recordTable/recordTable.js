/**
 * @file A LightningDatatable able to display UI API list view records.
 */
import { api } from 'lwc';
...
export default class RecordTable extends LightningDatatable {
    ...
    convertData() {
        this.data =
            this.state.listUi &&
            this.state.listUi.records.records.map((record) => {
                return Object.entries(record.fields).reduce((recordData, [apiName, { value }]) => {
                    recordData[apiName] =
                        apiName === this.state.nameField
                            ? {
    ...
    convertColumns() {
        this.columns = this.state.listUi && [
            ...this.state.listUi.info.displayColumns.map(({ fieldApiName, label }) => ({
                label,
